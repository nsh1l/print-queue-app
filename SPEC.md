...
Windowsでローカルの文書を印刷待ちにまとめ、選択したプリンターへ送信するスタンドアローンアプリ。
...
4. 利用可能なプリンターを一覧表示し、Windowsの既定プリンターを初期選択する。
5. 「選択したプリンターへ送信」は、待機中の各ファイルにWindows Shellの `printto` 動詞を使う。
6. 成功時は `送信済み`、開始できない場合は `エラー` を表示する。
...
- [ ] 選択削除とキュー全消去ができる。
- [ ] 一覧からMicrosoft Print to PDFを選び、1ファイルを送信して送信済み表示を確認できる。
